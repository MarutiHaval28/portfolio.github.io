...
void rentCar(std::vector<Car4>& cars);
void leaveFeedback(const std::vector<Car4>& cars);
void returnCar(std::vector<Car4>& cars);
...
void purchaseCar(vector<Car4>& cars)
{
    ...
    if (index >= 0 && index < cars.size() && cars[index].isAvailable)
    {
        ...
        generateInvoice(cars[index], "User", true, 0);
    }
    else
    {
...
void filterCars(const std::vector<Car4>& cars) {
    ...
    switch (choice) {
        ...
        case 3: {
            ...
            for (const auto& car : cars) {
                double carMileage = stod(car.mileage.substr(0, car.mileage.find("-")));
...
void showDealerLocation(const std::string& dealerName)
{
    ...
    {
        std::string url = "start chrome \"" + it->second + "\"";
        system(url.c_str());
    }
    else
    {
...
{
    const string filePathPurchase = "C:\\Users\\HP\\OneDrive\\Desktop\\Books_to_Read\\Purchase_and_booking.txt";
            ...
            while (true)
            {
                ...
                while (true)
                {
                    ...
                    switch (subChoice)
                    {
                        ...
                        break;
                        case 5:
                        bookCar(cars, carType);
                        break;
                        case 6:
                        cancelBooking(cars, carType);
                        break;
                        default:
